...
{
    if (!ui->lineEditEmail->isModified() || !ui->lineEditPassword->isModified()){
        const QString mess = "devi inserire una mail o password validi";
        QMessageBox::critical(this, tr("Miss data"), mess);
        return;
    }
    ...
    if (user == "admin@pas.com")
    {
        if (password == "admin")
        {
            ...
            file.close();
            QMessageBox::information(this, tr("Users"), stamp);
            clear();
            return;
        } else {
...
void ProjectQt::on_pushButtonRecovery_clicked()
{
    if (ui->lineEditEmail->isModified()){
        ...
        const QString mess = "inserisci l'email o numero di telefono";
        QMessageBox::critical(this, tr("Miss data"), mess);
    }
}
...
void ProjectQt::write()
{
    ...
    {
        const QString mess = field + " non valido.\nInserire una email (formato: <nome>@<dominio>.it) o numero di telefono (10 cifre) validi!";
...
void ProjectQt::validUser(QString field, QString password)
{
    ...
    const QString mess = "Email non presente! Effettua la registazione";
...
/**
 * Setta tutti i text delle lineEdit a vuoto in modo da pulire per eventuali futuri inserimenti di dati
 * @brief ProjectQt::clear
 */
void ProjectQt::clear() {
    ui->lineEditEmail->setText("");
    ui->lineEditPassword->setText("");
    ui->lineEditNewEmail->setText("");
    ui->lineEditNewPassword->setText("");
    ui->lineEditNewName->setText("");
    ui->lineEditNewSurname->setText("");
}
